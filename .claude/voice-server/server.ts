...
// Default voice ID (Marvin's voice)
const DEFAULT_VOICE_ID = process.env.ELEVENLABS_VOICE_ID || 'onwK4e9ZLuTAKqWW03F9';
...
async function generateSpeech(text: string, voiceId: string): Promise<ArrayBuffer> {
  ...
  if (!response.ok) {
    ...
    if (errorText.includes('model') || response.status === 422) {
      throw new Error(
        `ElevenLabs API error: Invalid model "${DEFAULT_MODEL}". Update ELEVENLABS_MODEL in ~/.env. See https://elevenlabs.io/docs/models`
      );
    }

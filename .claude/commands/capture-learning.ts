...
async function main() {
  ...
  if (!initialAssumption) {
    initialAssumption = await prompt(
      '🤔 What did we initially think was true/how we thought it worked? '
    );
  }
  ...
  if (!takeaway) {
    takeaway = await prompt(
      '🎯 What\'s the key takeaway ("So now we know..." or "So now we do it this way...")? '
    );
  }

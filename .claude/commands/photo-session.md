View photorealistic generation session history and recipes.
...
Session data location: `$PHOTO_DIR/`
...
```bash
cat "$PHOTO_DIR/YYYY-MM-DD.jsonl" | jq -r '. | "\(.timestamp | split("T")[1] | split(".")[0]) [\(.phase // "single")] \(.model) seed:\(.seed // "none")\n  \"\(.prompt | .[0:70])...\"\n  -> \(.output_image)"'
```

To list available dates:
```bash
ls -1 "$PHOTO_DIR"/*.jsonl 2>/dev/null | xargs -I{} basename {} .jsonl | sort -r
```

To list images for a date:
```bash
ls "$PHOTO_DIR/YYYY-MM-DD/"
```

To list saved recipes:
```bash
ls -1 "$PHOTO_DIR/recipes/"*.md 2>/dev/null | while read f; do
  name=$(basename "$f" .md)
...
bun ${PAI_DIR}/skills/Art/tools/generate-ulart-image.ts \
  --prompt-file "$PHOTO_DIR/recipes/RECIPE_NAME.md" \

...
import { toCSV, toJSON } from './lib/exporters.ts';
import { detectFormat, parseInput } from './lib/readers.ts';
import { addSheetWithData, loadTemplate, populateTemplate } from './lib/template.ts';
import {
  addDataToSheet,
  appendDataToSheet,
  createWorkbook,
    ...
    try {
      ...
      if (info.properties.title || info.properties.author) {
        ...
        if (info.properties.created) {
          console.log(`  Created: ${info.properties.created.toISOString().split('T')[0]}`);
        }
      }
    } catch (error) {
      console.error(`Error: ${error}`);
    ...
    try {
      ...
      if (options.addSheet) {
        if (options.data) {
          ...
          console.error(`Added sheet "${options.addSheet}" with ${data.length} rows`);
        } else {

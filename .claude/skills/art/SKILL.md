...
| Charcoal | #2D2D2D | {{DA}} signature, subtle details |
...
7. **NO TEXT** (except {{DA}} signature)
8. **KAI SIGNATURE:** Small subtle "{{DA}}" in charcoal (#2D2D2D) bottom right
...
**Nano Banana Pro supports 1K, 2K, and 4K resolution output.**
...
|------|------------------------|----------|
| **1K** | ~1408×768 (16:9) | Quick iteration, drafts |
| **2K** | ~2752×1536 (16:9) | Blog headers, social media |
| **4K** | ~5504×3072 (16:9) | Hero images, video backgrounds, print |
...
**Use a reference image to upscale low-resolution images while maintaining composition.**
...
bun run ${PAI_DIR}/skills/Art/tools/generate-ulart-image.ts \
  --model nano-banana-pro \
  --prompt "Upscale this image to higher resolution, maintaining the exact same composition, colors, and details." \
  --reference-image /path/to/low-res-image.png \
  --size 4K \
  --aspect-ratio 16:9 \
  --output /path/to/upscaled-image.png
```

### Upscaling Results

| Input Resolution | Output Resolution | Improvement |
|------------------|-------------------|-------------|
| 1408×768 (1K) | 5504×3072 (4K) | **~4x resolution** |

### Best Practices for Upscaling

1. **Match aspect ratio** — Use same aspect ratio as source image
2. **Specific prompt** — Describe what to preserve: "exact same composition, colors, style, all visual elements"
3. **Two-pass workflow** — Iterate on composition at 1K/2K (faster, cheaper), then upscale final to 4K

### Two-Pass Workflow Example

```bash
# Step 1: Iterate quickly at lower resolution
bun run ${PAI_DIR}/skills/Art/tools/generate-ulart-image.ts \
  --model nano-banana-pro \
...
# Step 2: Once composition is perfect, upscale to 4K
bun run ${PAI_DIR}/skills/Art/tools/generate-ulart-image.ts \
  --model nano-banana-pro \
  --prompt "Upscale this image to higher resolution, maintaining all visual details." \
  --reference-image /tmp/draft.png \
  --size 4K \
  --aspect-ratio 16:9 \
  --output /tmp/final-4k.png
```
...
**Optimal dimensions for creating animated videos from static images using Kling v2.5 Turbo Pro.**

### Why 2K Input (Not 4K)

Kling v2.5 Turbo Pro outputs at **1080p maximum** (1920×1080 for 16:9). Higher resolution inputs are downscaled internally, making 4K wasteful:

| Input Size | Kling Output | Notes |
|------------|--------------|-------|
| 1K (~1408×768) | 1920×1080 | Works, slight upscaling |
| **2K (~2752×1536)** | **1920×1080** | **Optimal - clean downscale** |
| 4K (~5504×3072) | 1920×1080 | Overkill - wasted resolution |

### Recommended I2V Workflow (Zero Conversions)

```
Nano Banana Pro (2K, 16:9) → Kling I2V → 1920×1080 MP4 → Deploy directly
```

**Step 1: Generate static image at 2K**
```bash
bun run ${PAI_DIR}/skills/Art/tools/generate-ulart-image.ts \
  --model nano-banana-pro \
  --prompt "[YOUR HERO IMAGE PROMPT]" \
  --size 2K \
  --aspect-ratio 16:9 \
  --output /tmp/hero-static.png
```

**Step 2: Animate with Kling**
```bash
bun run ${PAI_DIR}/skills/Art/tools/generate-ulart-video.ts \
  --model kling \
  --image /tmp/hero-static.png \
  --prompt "subtle atmospheric movement, gentle parallax" \
  --duration 10 \
  --tail-image /tmp/hero-static.png \
  --output /path/to/output/intro.mp4
```

**Step 3: Deploy** — No conversion needed

### Why No Conversion?

Modern CSS uses `object-fit: cover` which scales any aspect ratio video to fill its container without distortion. Most themes (including Snook/Astro themes) handle 16:9 videos natively.

| CSS Property | Effect |
|--------------|--------|
| `object-fit: cover` | Scales to fill, crops edges if needed |
| `width/height: 100%` | Responsive to container |
| `position: absolute` | Full container fill |

### I2V Dimension Summary

| Stage | Dimensions | Aspect Ratio | Rationale |
|-------|------------|--------------|-----------|
| Nano Banana Pro input | ~2752×1536 | 16:9 | High quality, not wasteful |
| Kling output | 1920×1080 | 16:9 | Model's max resolution |
| Theme display | Responsive | Any | CSS handles scaling |
...
Sign "{{DA}}" small in charcoal (#2D2D2D) bottom right corner.
NO other text.
```

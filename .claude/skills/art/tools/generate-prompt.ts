...
const _ART_AESTHETIC_PATH = resolve(process.env.HOME!, '.claude/skills/CORE/art-aesthetic.md');
...
function selectComposition(essayContent: string, override?: CompositionType): CompositionType {
  ...
  if (contentLower.includes('together') || contentLower.includes('collaborate')) {
    return 'dialogue';
  }
  if (contentLower.includes('build') || contentLower.includes('create')) {
    return 'workshop';
  }
...
function selectColors(essayContent: string, override?: string): TokyoNightColor[] {
  if (override) {
  ...
  if (contentLower.includes('tool') || contentLower.includes('productivity')) {
    return ['Bright Cyan'];
  }
  if (contentLower.includes('human') || contentLower.includes('growth')) {
    return ['Neon Green'];
  }
...
function selectMotifs(essayContent: string, override?: string): Human3Motif[] {
  if (override) {
...
function main() {
  ...
  const coreObjectDescription = buildVisualMetaphor(essayContent, compositionType, characterFocus);

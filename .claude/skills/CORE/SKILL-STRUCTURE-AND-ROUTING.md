...
**Canonical reference for {{DA}} skill organization, routing patterns, ecosystem, and management**
...
In {{DA}}'s architecture, **skills are the primary organizational primitive** - self-contained packages of context, workflows, and capabilities that enable specialized functionality.
...
- {{DA}} activates `research` skill → Loads context from research/SKILL.md
...
- {{DA}} reads workflow file and executes steps
...
Based on analysis of 29 production skills in {{DA}}, three clear organizational patterns emerged:
...
3. **Directory Scanning**
   ...
   ```
   → {{DA}} scans directory and presents options
...
4. **Paired Documentation**
   ...
   ```
   → {{DA}} reads .md for instructions, executes .ts for automation
...
Add to `${PAI_DIR}/mcp_settings.json` skills section:
...
Expected: {{DA}} activates skill-name, loads SKILL.md
```
...
**Quick Reference: All {{DA}} Skills**

This section provides a comprehensive directory of all skills in the {{DA}} ecosystem, organized by category with capabilities and usage guidance.

### Overview

{{DA}} is organized around **topic-based skills** following the Skills-as-Containers architecture. Each skill is a self-contained domain with workflows, documentation, and references organized using **progressive disclosure** - load only what you need, when you need it.
...
**Domain:** {{DA}} identity, contacts, preferences, security, architecture
**Key Capabilities:**
...
**When to use:** Automatically loaded - provides foundation for all {{DA}} operations
**Location:** `${PAI_DIR}/skills/CORE/`

---

**system**
Complete {{DA}} system infrastructure and operational tooling.
...
- **Anthropic Tracking** - Monitor product changes, actionable recommendations for {{DA}}
...
```
User request → Main Agent ({{DA}}) analyzes intent
             ↓
...
**Skills:** {{DA}}-specific domain knowledge
...
Complete framework for creating new skills or updating existing ones, following Anthropic standards and {{DA}}-specific patterns.

**Quick Start:**
```bash
# Automated skill creation (recommended)
${PAI_DIR}/skills/Createskill/scripts/init-skill.sh my-skill --type simple
${PAI_DIR}/skills/Createskill/scripts/init-skill.sh my-skill --type complex
${PAI_DIR}/skills/Createskill/scripts/init-skill.sh my-skill --type with-agents
```
...
- Self-contained but inherits {{DA}} context
...
2. **Relevance to Daniel's Needs**
   - Matches {{DA}} technology stack (TypeScript, bun, modern tools)
...
3. **Integration Compatibility**
   - Works with existing {{DA}} infrastructure
...
[Skills that solve current {{DA}} gaps or enhance existing workflows]
...
3. Adapt to {{DA}} patterns (TypeScript, structured format, etc.)
...
Based on Daniel's {{DA}} needs, prioritize these categories:
...
- No duplication of existing {{DA}} capabilities
...
3. **Adapt** - Modify for {{DA}} patterns if needed
...
1. **Creation**: Follow Anthropic standards + {{DA}} patterns
...
5. **Integration**: Ensure {{DA}} compatibility
...
- `${PAI_DIR}/skills/CORE/CONSTITUTION.md` - Overall {{DA}} architecture and philosophy

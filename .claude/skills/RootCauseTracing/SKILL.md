...
```bash
${PAI_DIR}/skills/RootCauseTracing/find-polluter.sh '.git' 'src/**/*.test.ts'
```
...
3. No → Keep tracing → Eventually find source
4. Fix at source → Add validation at each layer → Bug impossible
...
- `systematic-debugging` - Broader debugging framework

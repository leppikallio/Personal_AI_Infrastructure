...
Create a professionally-formatted Word document from Markdown content using the docx-cli.

## Prerequisites

- docx-cli installed at `${PAI_DIR}/bin/docx-cli/`
- Template file at `~/orbit_doc_template.dotx` (or specify custom path)
...
Ensure markdown content is ready. Can come from:
...
**From file:**
```bash
bun ${PAI_DIR}/bin/docx-cli/docx-cli.ts create <input.md> -o <output.docx> \
  --title "Document Title" \
  --author "Author Name" \
  --date "2025-01-15" \
  --doc-version "1.0" \
...
```bash
echo "${markdown_content}" | bun ${PAI_DIR}/bin/docx-cli/docx-cli.ts create - -o <output.docx> \
  --title "Document Title" \
  --author "Author Name"
```
...
bun ${PAI_DIR}/bin/docx-cli/docx-cli.ts create quarterly-report.md \
